...
[![Tests](https://img.shields.io/badge/tests-1002%20green-brightgreen)](#project-infrastructure)
...
**Wave K: TUI scrolling** (0.48.0): dragging the journal scrollbar thumb now follows the pointer until button release — even when the cursor leaves the narrow track column (the drag used to be lost). Track click jumps proportionally, ▲/▼ step; drag coordinates clamp to the track. (Wave J — 0.47.0 — was catalog-only: the `security-pentest-ru` skill in berimor-skills, no core release.)

**Wave M: visual DAG-builder** (frontend, no core release): `tools/dag-builder.html` — a single-file process builder (step nodes of every kind, seq/branch/parallel edges, field form exactly matching StepKind, graph validation, berimor YAML export, JSON project save/load). The format sample — `fixtures/golden/processes/dag-builder/` — parses with the production binary.
