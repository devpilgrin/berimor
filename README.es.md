...
[![Tests](https://img.shields.io/badge/tests-1003%20green-brightgreen)](#infraestructura-del-proyecto)
...
**SGR: el esquema guía el razonamiento** (0.30.0): un contrato puede declarar campos de justificación ANTES de los campos objetivo — `risk_factors` (lista no vacía) antes de `risk` en `ClassificationOut`; tras enumerar los factores, el modelo asigna la puntuación con fundamento en lugar de arbitrariamente. El orden de los campos en el JSON Schema sigue el orden de declaración (schemars `preserve_order`). En proveedores con constrained decoding (`response_format = "json_schema"` en `[[providers]]`: compatibles con OpenAI, Ollama vía `format`, llama.cpp), el orden de generación queda físicamente impuesto por el esquema — el modelo no puede emitir el número sin rellenar antes los factores. En proveedores sin constrained decoding (DeepSeek, Kimi — solo `json_object`) se aplica el nivel blando: orden de campos en el prompt + obligatoriedad por esquema + validación de la mediación. Regla para contratos de configuración: declarar los campos de justificación antes que los objetivo. El llama.cpp autónomo (in-process) impone el orden mediante una gramática GBNF construida desde el esquema del contrato (0.31.0).

**Ola E: memoria** (0.42.0): adaptador Qdrant para la capa Facts — `[memory] qdrant_url = "http://127.0.0.1:6333"` (+ `qdrant_collection`, `qdrant_api_key_env`): búsqueda semántica sobre índice HNSW en lugar de un escaneo completo de SQLite (HTTP/JSON puro, sin cliente gRPC); upsert/scroll/cosine/hybrid/delete verificados en vivo contra un Qdrant real. Caché de respuestas del modelo por hash EXACTO — `[agent] response_cache = true`: una llamada repetida con la misma entrada no llega al proveedor (un acierto no escribe usage — no hubo llamada); almacenamiento en `<storage>.cache.db` separado (borrarlo = invalidación); la caché por similitud de embeddings no se construye deliberadamente (no determinismo de respuestas).

**Ola D: reglas Rego del gate** (0.41.0): una política OPA/Rego externa sobre las reglas estáticas del capability-gate — vía regorus (in-process, sin sidecar). `[gate] rego_policy = "policy.rego"` + `environment = "prod"`: la política (`package berimor`, `deny contains msg if { ... }`) ve `input.tool`, `input.args`, `input.mutates`, `input.environment` y solo puede prohibir más estricto que la estática — nunca permitir más débil, el núcleo sigue siendo determinista. Error de parseo = fallo de arranque, error de evaluación = fail-closed. El ejemplo solicitado funciona: «terminal.exec prohibido en el entorno prod».

**Ola F: berimor como GitHub App** (0.43.0): `berimor serve` acepta webhooks de GitHub en `POST /webhooks/github` — verificación HMAC-SHA256 (secreto del webhook), JWT RS256 → token de instalación; una marca disparadora en un comentario (`/berimor` por defecto) ejecuta el proceso de forma no interactiva y devuelve el resultado como comentario en el issue/PR. 202 inmediato, proceso en segundo plano. Config `[github_app]`: app_id, private_key_path, process, trigger.

**Ola G: ACP — berimor en el editor** (0.43.0→0.44.0): `berimor acp` habla el Agent Client Protocol por stdio — Zed y editores compatibles conectan berimor como agente externo. Una sesión es una ejecución del proceso de `[acp] process`: un prompt ejecuta el proceso (entrada `{text}`), los eventos del journal se transmiten al editor (turnos de herramientas, gates, rechazos de mediación), la respuesta llega al final; cancel deja de esperar mientras la ejecución termina en el journal. La salida del proceso va a stderr en modo ACP — stdout pertenece al protocolo.

**Ola H: red en la sandbox** (0.45.0): `[sandbox] network = "restrict"` + `allow_connect_ports`/`allow_bind_ports` — los subprocesos terminal.exec/terminal.start reciben reglas de red Landlock (ABI 4, núcleo 6.7+): TCP connect/bind solo a puertos listados, el resto lo rechaza el núcleo (EACCES). En núcleos antiguos: auto — aviso y omisión, require — fail-closed. Por defecto `off` — comportamiento sin cambios.

**Ola I: journal federado** (0.46.0): `berimor journal export <instance> --out <file>` / `import <file>` — llevar una ejecución entre máquinas como JSON portable con huella sha256. La importación verifica el hash (archivo dañado rechazado), renombra ante colisión de id (`-imported-N`) — es imposible escribir en una instancia existente; las marcas de tiempo se preservan.

**Ola K: desplazamiento TUI** (0.48.0): arrastrar el control de la barra del journal sigue al puntero hasta soltar el botón — incluso fuera de la estrecha columna (antes el arrastre se perdía). Clic en la pista = salto proporcional, ▲/▼ = paso; las coordenadas se limitan a la pista. (Ola J — 0.47.0 — solo catálogo: skill `security-pentest-ru` en berimor-skills, sin release del núcleo.)

**Ola M: DAG-builder visual** (frontend, sin release del núcleo): `tools/dag-builder.html` — editor de procesos en un solo archivo (nodos de todos los tipos, aristas seq/branch/parallel, formulario según StepKind, validación del grafo, exportación YAML berimor, proyecto JSON). Muestra: `fixtures/golden/processes/dag-builder/` — parseada por el binario de producción.

**Ola C: LLM-as-a-Judge** (0.40.0): `berimor eval <dir> --judge` — tras la ejecución del conjunto dorado, un proveedor fuerte (primero en el orden de failover) puntúa el estado final de cada escenario terminado: una puntuación de 1-5 y su justificación se escriben como evento `judge_score` en el diario del escenario y se muestran. Criterios desde `<escenario>.judge.md` (o rúbrica por defecto: completitud, precisión, sin hechos inventados, forma). `--judge-threshold <N>` — puerta CI: media por debajo del umbral = error del comando. La respuesta del juez se lee con la misma reparación EOF de la mediación; los escenarios no terminados se omiten honestamente.

**Ola B: observabilidad** (0.39.0): `berimor otlp <run> --endpoint <url>` — una ejecución de proceso como traza en OTLP/HTTP JSON: span raíz de la ejecución, span por nodo del grafo, span de llamada LLM (latencia + tokens como atributos), human_gate (intervalo hasta respuesta/timeout), turnos de herramientas del bucle libre. traceId/spanId son deterministas (reexportación idempotente). Aceptado por los colectores Jaeger y Grafana Tempo (puerto 4318) y Langfuse — un solo OTLP, sin exportadores dedicados; cabeceras de autenticación con `--header 'Name: value'`.

**Ola A: resiliencia y coste** (0.38.0): circuit breaker en el Model Pool — N fallos de transporte consecutivos abren el automático, el proveedor se omite hasta una prueba semiabierta tras el cooldown, con alerta visible «<nombre> → circuit-open» (`[agent] breaker_failures`, `breaker_cooldown_secs`; 0 = desactivado). Atribución de costes: cada llamada al modelo registra el uso (tokens, latencia, paso — evento `model_usage`); el llama.cpp local cuenta tokens con su tokenizador; `berimor cost <run>` — informe por paso y total (precios desde `cost_per_1k_tokens` del proveedor; sin precio — tokens honestos, sin cifras inventadas).

**Capa de reglas y berimor como servidor MCP** (0.37.0, tras Harness AI 3.0): (1) **reglas** — estándares Markdown de `~/.config/berimor/rules/` y `.berimor/rules/` se inyectan en el contexto de cada paso con modelo ANTES de generar (capa blanda; la dura sigue siendo la mediación); las de proyecto prevalecen sobre las globales; (2) **`berimor mcp-serve`** — servidor MCP por stdio: agentes y editores externos ejecutan procesos berimor vía `process.list`/`process.run`/`trace.read` — el modelo piensa fuera, el código decide dentro; (3) **GitHub Action** `devpilgrin/berimor-action@v1` — procesos como pasos de CI.

**Tomado de DeepSeek Harness** (0.36.0): (1) **poda de observaciones** — los resultados largos de herramientas se recortan en el prompt (cabeza+marcador+cola, el original queda en el journal; `[agent] tool_result_max_chars`, 0 = off); (2) **sandbox Landlock** para `terminal.exec`/`terminal.start` — implementación propia con libc (sin binario externo): el subproceso físicamente no puede salir del workspace, los directorios del sistema son de solo lectura; `[sandbox] landlock = off|auto|require`, require es fail-closed; (3) **compactación del chat** — el historial que supera el umbral se resume en una nota por el proveedor principal, la cola queda literal, un fallo de resumen nunca rompe el turno (`[agent] compact_threshold_chars`, 0 = off).

**Resistencia a la generación truncada** (0.35.2): un modelo local que llega al tope de tokens corta el JSON («EOF while parsing») — antes quemaba 3 reintentos y detenía el proceso con una escalada. Ahora la etapa parse de la mediación completa el truncamiento estructuralmente (comillas/llaves de cierre; contenido intacto; la basura sigue rechazada), la reparación queda en el journal (`mediation_parse_repaired`) — los reintentos y la escalada quedan para errores reales. El contexto del proveedor local sube a 8192 y es configurable (`local_ctx_tokens`).

**Presupuesto de turnos del bucle libre** (0.34.0): tope por mensaje — `[agent] max_turns` (32 por defecto, antes 12). La protección contra bucles está separada del límite de longitud: repetir la misma acción (herramienta + argumentos idénticos) genera una advertencia en el prompt, cuatro seguidas detienen con un error `StuckLoop` explícito; el trabajo largo VARIADO (análisis de proyecto con decenas de lecturas) no se penaliza. Al ~20% del tope, el motor añade al prompt la nota «quedan N turnos — resume el resultado en Finish».

**Pentest con validación PoC** (0.33.0, inspirado en usestrix/strix): proceso de referencia [`fixtures/golden/processes/pentest/`](fixtures/golden/processes/pentest/) — reconocimiento → hipótesis (evidence antes que class, SGR) → `human_gate` → verificación activa → informe donde un hallazgo solo se acepta con prueba de ejecución; lo no confirmado va honestamente a `unconfirmed`. Las salvaguardas son obligatorias: objetivos desde un scope explícito, acciones activas vía un humano, todo en el journal. Además: un deny estático de la capa capability ahora es una observación de turno en el bucle libre en lugar de abortar la ejecución — el modelo corrige la acción según las reglas y el gate sigue bloqueando cada intento.

**Gobernanza de extensiones** (0.32.0): `berimor skill lint` / `berimor agent lint` — comprobaciones estáticas del manifiesto (contrato de nombre, herramientas conocidas, coherencia de `permissions` — net/exec/fs-write/spawn — con el techo de tools); las instalaciones desde el catálogo son fail-closed: un error de lint revierte. `berimor skill review` / `agent review` — revisión multimodelo del contenido como datos no confiables: cada proveedor configurado emite un veredicto independiente, el resultado por cuórum (un fail = fail), informe JSON con hallazgos. Los releases incluyen `release-evidence.json` (hashes, firmas, SBOM, rastro CI) y `release-smoke-linux-x64.json`.
