...
**Фаза 6 (Memory)** также закрыта — рабочая память со сворачиванием при переполнении бюджета, эпизодическая память с полнотекстовым поиском (FTS5), семантическая память с дедупликацией фактов и конфликт-событиями, гибридный векторно-текстовый поиск (`sqlite-vec`), процедурная память (формат файла навыка), граф сущностей (entity resolution, типизированные контракты — прошёл обязательное независимое ревью) и изоляция профилей/арендаторов. Пока не подключено к `berimor run` через Context Engine — компоненты реализованы и протестированы, интеграция в реальный прогон CLI впереди.

Ещё не реализовано: Actors & Scheduler (Фаза 7), Tool Runtime/MCP (Фаза 8), Eval & Observability (Фаза 9), исполнители `CodeAct`/`AgentStep` (E6–E9) — см. `docs/ROADMAP.md` для полного графа задач.
