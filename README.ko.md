...
[![Tests](https://img.shields.io/badge/tests-1003%20green-brightgreen)](#프로젝트-인프라)
...
| `/mouse` | 마우스 토글: 캡처 시 — 휠로 저널 스크롤(오른쪽에 위치 표시 스크롤바), 저널 클릭으로 스크롤 포커스; 해제 시 — 선택 모드: 정보 패널 숨김, 저널 전체 너비, 네이티브 선택은 저널만 포함(캡처 시 선택은 Shift 사용) |
...
**웨이브 E: 메모리** (0.42.0): Facts 계층 Qdrant 어댑터 — `[memory] qdrant_url = "http://127.0.0.1:6333"`(+ `qdrant_collection`, `qdrant_api_key_env`): SQLite 전체 스캔 대신 HNSW 인덱스 시맨틱 검색(순수 HTTP/JSON, gRPC 클라이언트 없음); upsert/scroll/cosine/hybrid/delete 실제 Qdrant로 실측. 정확 해시 모델 응답 캐시 — `[agent] response_cache = true`: 동일 입력 반복 호출은 프로바이더에 닿지 않음(히트는 usage 미기록 — 호출 자체가 없음); 저장소는 별도 `<storage>.cache.db`(삭제=무효화); 임베딩 유사도 캐시는 의도적으로 만들지 않음(응답 비결정성).
...
**웨이브 I: 페더레이티드 저널** (0.46.0): `berimor journal export <instance> --out <file>` / `import <file>` — sha256 지문이 포함된 이식 가능 JSON으로 실행을 머신 간 이전. 임포트는 해시를 검증(손상 파일 거부), id 충돌 시 리네임(`-imported-N`) — 기존 인스턴스에 추가 기록 불가; 타임스탬프는 원본 유지.
...
**검증 규율.** 모든 릴리스: `cargo fmt` + `clippy -D warnings` + `cargo test --workspace`(1003개 테스트: 유닛, 통합, 실제 바이너리를 통한 e2e, 프로세스 및 악의적 입력의 골든 픽스처). 중요 컴포넌트는 필수 독립 리뷰를 거칩니다. 전체 독립 감사(`docs/audit-2026-07-31.md`) — **모든 지적 사항이 해결되었거나 의식적으로 문서화됨**.
