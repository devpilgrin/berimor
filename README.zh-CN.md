...
[![Tests](https://img.shields.io/badge/tests-1003%20green-brightgreen)](#项目基础设施)
...
**G 波：ACP——编辑器中的 berimor**（0.44.0）：`berimor acp` 通过 stdio 使用 Agent Client Protocol——Zed 及兼容编辑器可将 berimor 作为外部代理接入。会话即 `[acp] process` 的流程运行：prompt 运行流程（输入 `{text}`），日志事件流式推送到编辑器（工具调用、门禁、调解拒绝），完成后返回结果；cancel 停止等待，运行在日志中完成。ACP 模式下流程输出走 stderr——stdout 属于协议。
...
**M 波：可视化 DAG 构建器**（前端，无核心发布）：`tools/dag-builder.html`——单文件流程编辑器（全部类型的步骤节点、seq/branch/parallel 连线、严格匹配 StepKind 的字段表单、图校验、berimor YAML 导出、JSON 项目）。格式样例 `fixtures/golden/processes/dag-builder/` 可被生产二进制解析。
...
**检查纪律。** 每次发布：`cargo fmt` + `clippy -D warnings` + `cargo test --workspace`（1003 个测试：单元、集成、通过真实二进制的 e2e、流程和恶意输入的黄金夹具）。关键组件必须通过强制性的独立评审。完整的独立审计（`docs/audit-2026-07-31.md`）——**所有发现均已关闭或有意识地记录在案**。
