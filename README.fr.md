...
[![Tests](https://img.shields.io/badge/tests-1003%20green-brightgreen)](#infrastructure-du-projet)
...
**Vague E : mémoire** (0.42.0) : adaptateur Qdrant pour la couche Facts — `[memory] qdrant_url = "http://127.0.0.1:6333"` (+ `qdrant_collection`, `qdrant_api_key_env`) : recherche sémantique sur index HNSW au lieu d'un scan complet SQLite (HTTP/JSON pur, pas de client gRPC) ; upsert/scroll/cosine/hybrid/delete vérifiés en direct contre un vrai Qdrant. Cache des réponses du modèle par hachage EXACT — `[agent] response_cache = true` : un appel répété avec la même entrée n'atteint pas le fournisseur (un hit n'écrit pas d'usage — il n'y a pas eu d'appel) ; stockage dans un `<storage>.cache.db` séparé (suppression = invalidation) ; le cache par similarité d'embeddings n'est volontairement pas construit (non-déterminisme des réponses).

**Vague D : règles Rego du gate** (0.41.0) : une politique OPA/Rego externe au-dessus des règles statiques du capability-gate — via regorus (in-process, sans sidecar). `[gate] rego_policy = "policy.rego"` + `environment = "prod"` : la politique (`package berimor`, `deny contains msg if { ... }`) voit `input.tool`, `input.args`, `input.mutates`, `input.environment` et ne peut qu'interdire plus strictement que la statique — jamais autoriser plus faiblement, le noyau reste déterministe. Erreur de parsing = échec au démarrage, erreur d'évaluation = fail-closed. L'exemple demandé fonctionne : « terminal.exec interdit en environnement prod ».

**Vague F : berimor comme GitHub App** (0.43.0) : `berimor serve` accepte les webhooks GitHub sur `POST /webhooks/github` — vérification HMAC-SHA256 (secret du webhook), JWT RS256 → jeton d'installation ; un marqueur déclencheur dans un commentaire (`/berimor` par défaut) exécute le processus en non-interactif et renvoie le résultat en commentaire issue/PR. 202 immédiat, processus en arrière-plan. Config `[github_app]` : app_id, private_key_path, process, trigger.

**Vague G : ACP — berimor dans l'éditeur** (0.44.0) : `berimor acp` parle l'Agent Client Protocol via stdio — Zed et les éditeurs compatibles connectent berimor comme agent externe. Une session est un run du processus `[acp] process` : un prompt lance le processus (entrée `{text}`), les événements du journal sont diffusés dans l'éditeur (tours d'outils, gates, rejets de médiation), la réponse arrive à la fin ; cancel interrompt l'attente, le run se termine dans le journal. La sortie du processus passe sur stderr en mode ACP — stdout appartient au protocole.

**Vague H : réseau dans le bac à sable** (0.45.0) : `[sandbox] network = "restrict"` + `allow_connect_ports`/`allow_bind_ports` — les sous-processus terminal.exec/terminal.start reçoivent des règles réseau Landlock (ABI 4, noyau 6.7+) : TCP connect/bind uniquement vers les ports listés, le reste est refusé par le noyau (EACCES). Noyau ancien : auto — avertissement et repli, require — fail-closed. Défaut `off` — comportement inchangé.

**Vague I : journal fédéré** (0.46.0) : `berimor journal export <instance> --out <file>` / `import <file>` — transporter un run entre machines en JSON portable avec empreinte sha256. L'import vérifie le hash (fichier corrompu rejeté), renomme en cas de collision d'id (`-imported-N`) — impossible d'écrire dans une instance existante ; les horodatages sont préservés.

**Vague K : défilement TUI** (0.48.0) : le glissement du curseur de la barre de défilement du journal suit le pointeur jusqu'au relâchement — même hors de l'étroite colonne (le drag se perdait auparavant). Clic sur la piste = saut proportionnel, ▲/▼ = pas ; les coordonnées du drag sont bornées à la piste. (Vague J — 0.47.0 — catalogue uniquement : skill `security-pentest-ru` dans berimor-skills, sans release du noyau.)

**Vague M : DAG-builder visuel** (frontend, sans release du noyau) : `tools/dag-builder.html` — éditeur de processus en un seul fichier (nœuds de tous types, arêtes seq/branch/parallel, formulaire conforme à StepKind, validation du graphe, export YAML berimor, projet JSON). Échantillon : `fixtures/golden/processes/dag-builder/` — parsé par le binaire de production.

**Vague C : LLM-as-a-Judge** (0.40.0) : `berimor eval <dir> --judge` — après l'exécution du jeu doré, un fournisseur fort (premier dans l'ordre de failover) note l'état final de chaque scénario terminé : un score de 1 à 5 et une justification sont écrits comme événement `judge_score` dans le journal du scénario et affichés. Critères via `<scénario>.judge.md` (sinon rubrique par défaut : complétude, exactitude, aucun fait inventé, forme). `--judge-threshold <N>` — porte CI : moyenne sous le seuil = échec de la commande. La réponse du juge est lue via la même réparation EOF de la médiation ; les scénarios non terminés sont honnêtement ignorés.

**Vague B : observabilité** (0.39.0) : `berimor otlp <run> --endpoint <url>` — une exécution de processus comme trace en OTLP/HTTP JSON : span racine de l'exécution, span par nœud du graphe, span d'appel LLM (latence + tokens en attributs), human_gate (intervalle jusqu'à réponse/timeout), tours d'outils de la boucle libre. traceId/spanId sont déterministes (ré-export idempotent). Accepté par les collecteurs Jaeger et Grafana Tempo (port 4318) et Langfuse — un seul OTLP, pas d'exportateurs dédiés ; en-têtes d'authentification via `--header 'Name: value'`.

**Vague A : résilience et coût** (0.38.0) : circuit breaker dans le Model Pool — N échecs de transport consécutifs ouvrent le disjoncteur, le fournisseur est ignoré jusqu'à une sonde semi-ouverte après le cooldown, avec une alerte visible « <nom> → circuit-open » (`[agent] breaker_failures`, `breaker_cooldown_secs` ; 0 = désactivé). Attribution des coûts : chaque appel de modèle journalise l'usage (tokens, latence, étape — événement `model_usage`) ; le llama.cpp local compte les tokens via son tokenizer ; `berimor cost <run>` — rapport par étape et total (prix depuis `cost_per_1k_tokens` du fournisseur ; sans prix — des tokens honnêtes, pas de montants inventés).

**Couche de règles et berimor comme serveur MCP** (0.37.0, d'après Harness AI 3.0) : (1) **règles** — des standards Markdown de `~/.config/berimor/rules/` et `.berimor/rules/` sont injectés dans le contexte de chaque étape avec modèle AVANT la génération (couche souple ; la couche dure reste la médiation) ; les règles de projet priment sur les globales ; (2) **`berimor mcp-serve`** — serveur MCP sur stdio : des agents et éditeurs externes pilotent les processus berimor via `process.list`/`process.run`/`trace.read` — le modèle pense dehors, le code décide dedans ; (3) **GitHub Action** `devpilgrin/berimor-action@v1` — les processus comme étapes CI.

**Emprunté à DeepSeek Harness** (0.36.0) : (1) **élagage des observations** — les longs résultats d'outils sont tronqués dans le prompt (tête+marqueur+queue, l'original reste dans le journal ; `[agent] tool_result_max_chars`, 0 = off) ; (2) **bac à sable Landlock** pour `terminal.exec`/`terminal.start` — implémentation libc propre (sans binaire externe) : le sous-processus ne peut physiquement pas quitter l'espace de travail, les répertoires système sont en lecture seule ; `[sandbox] landlock = off|auto|require`, require est fail-closed ; (3) **compaction du chat** — l'historique au-delà du seuil est résumé en note par le provider principal, la queue reste verbatim, un échec de résumé ne fait jamais échouer le tour (`[agent] compact_threshold_chars`, 0 = off).

**Résilience aux générations tronquées** (0.35.2) : un modèle local qui atteint le plafond de tokens coupe le JSON (« EOF while parsing ») — cela brûlait 3 tentatives et arrêtait le processus par escalade. Désormais, l'étape parse de la médiation complète la troncature structurellement (guillemets/accolades fermants ; contenu intact ; le rebut reste rejeté), la réparation est journalisée (`mediation_parse_repaired`) — les tentatives et l'escalade restent pour les vraies erreurs. Le contexte du provider local passe à 8192 et se configure (`local_ctx_tokens`).

**Budget de tours de la boucle libre** (0.34.0) : plafond par message — `[agent] max_turns` (32 par défaut, 12 avant). La protection anti-boucle est séparée du plafond de longueur : la répétition de la même action (outil + arguments identiques) déclenche un avertissement dans le prompt, quatre d'affilée arrêtent avec une erreur `StuckLoop` explicite ; le long travail VARIÉ (analyse de projet sur des dizaines de lectures) n'est pas pénalisé. À ~20% du plafond, le moteur ajoute au prompt une note « il reste N tours — synthétise le résultat dans Finish ».

**Pentest avec validation PoC** (0.33.0, d'après usestrix/strix) : processus de référence [`fixtures/golden/processes/pentest/`](fixtures/golden/processes/pentest/) — reconnaissance → hypothèses (evidence avant class, SGR) → `human_gate` → vérification active → rapport où une constatation n'est acceptée qu'avec une preuve d'exécution ; le non-confirmé aboutit honnêtement dans `unconfirmed`. Les garde-fous sont obligatoires : cibles issues d'un scope explicite, actions actives via un humain, tout est journalisé. De plus : un deny statique de la couche capability devient dans la boucle libre une observation de tour au lieu d'arrêter l'exécution — le modèle corrige l'action selon les règles, le gate bloque toujours chaque tentative.

**Gouvernance des extensions** (0.32.0) : `berimor skill lint` / `berimor agent lint` — vérifications statiques du manifeste (contrat de nom, outils connus, cohérence des `permissions` — net/exec/fs-write/spawn — avec le plafond d'outils) ; les installations depuis le catalogue sont fail-closed : une erreur de lint annule. `berimor skill review` / `agent review` — revue multi-modèles du contenu comme données non fiables : chaque provider configuré rend un verdict indépendant, le résultat par quorum (un fail = fail), rapport JSON avec les constats. Les releases embarquent `release-evidence.json` (hachés, signatures, SBOM, trace CI) et `release-smoke-linux-x64.json`.
