...
[![Tests](https://img.shields.io/badge/tests-1003%20green-brightgreen)](#projektinfrastruktur)
...
**Welle F: berimor als GitHub App** (0.43.0): `berimor serve` nimmt GitHub-Webhooks auf `POST /webhooks/github` entgegen — HMAC-SHA256-Verifizierung (Webhook-Secret), RS256-JWT → Installation-Token; eine Trigger-Markierung im Kommentar (Standard `/berimor`) startet den Prozess nicht-interaktiv und meldet das Ergebnis als Issue/PR-Kommentar zurück. 202 sofort, der Prozess läuft im Hintergrund. `[github_app]`-Konfig: app_id, private_key_path, process, trigger.

**Welle G: ACP — berimor im Editor** (0.44.0): `berimor acp` spricht das Agent Client Protocol über stdio — Zed und kompatible Editoren binden berimor als externen Agenten ein. Eine Sitzung ist ein Prozesslauf aus `[acp] process`: ein Prompt startet den Prozess (Eingabe `{text}`), Journal-Ereignisse streamen in den Editor (Tool-Züge, Gates, Mediation-Ablehnungen), die Antwort kommt am Ende; cancel beendet das Warten, der Lauf endet im Journal. Prozessausgaben gehen im ACP-Modus nach stderr — stdout gehört dem Protokoll.

**Welle H: Netzwerk in der Sandbox** (0.45.0): `[sandbox] network = "restrict"` + `allow_connect_ports`/`allow_bind_ports` — terminal.exec/terminal.start-Subprozesse erhalten Landlock-Netzwerkregeln (ABI 4, Kernel 6.7+): TCP connect/bind nur für gelistete Ports, alles andere lehnt der Kernel ab (EACCES). Auf älteren Kerneln: auto — Warnung und Überspringen, require — fail-closed. Standard `off` — Verhalten unverändert.

**Welle I: föderatives Journal** (0.46.0): `berimor journal export <instance> --out <file>` / `import <file>` — einen Lauf zwischen Maschinen als portables JSON mit sha256-Fingerabdruck tragen. Import prüft den Hash (beschädigte Dateien werden abgelehnt), benennt bei id-Kollision um (`-imported-N`) — Anhängen an eine bestehende Instanz ist unmöglich; Zeitstempel bleiben original.

**Welle K: TUI-Scrollen** (0.48.0): Das Ziehen des Journal-Scrollbalkens folgt dem Zeiger bis zum Loslassen — auch außerhalb der schmalen Spalte (früher ging der Drag verloren). Klick auf die Leiste springt proportional, ▲/▼ schrittweise; Drag-Koordinaten werden auf die Leiste geklemmt. (Welle J — 0.47.0 — war reiner Katalog: Skill `security-pentest-ru` in berimor-skills, kein Core-Release.)

**Welle M: visueller DAG-Builder** (Frontend, kein Core-Release): `tools/dag-builder.html` — eindateiliger Prozess-Editor (Schritt-Knoten aller Typen, seq/branch/parallel-Kanten, Feldform exakt nach StepKind, Graph-Validierung, berimor-YAML-Export, JSON-Projekt). Formatbeispiel: `fixtures/golden/processes/dag-builder/` — vom Produktiv-Binary geparst.
...
**Prüfdisziplin.** Jedes Release: `cargo fmt` + `clippy -D warnings` + `cargo test --workspace` (1003 Tests: Unit-, Integrations-, E2E-Tests über das echte Binary, goldene Fixtures von Prozessen und bösartigen Eingaben). Kritische Komponenten durchlaufen ein obligatorisches unabhängiges Review. Ein vollständiges eigenständiges Audit (`docs/audit-2026-07-31.md`) — **alle Befunde sind geschlossen oder bewusst dokumentiert**.
