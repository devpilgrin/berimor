...
pub enum EventKind {
    ...
    /// Оценка судьи (волна C, 0.40.0): сильный провайдер оценил финальный
    /// результат сценария eval по рубрике; скор 1..5 и обоснование.
    JudgeScore {
        scenario: String,
        score: f64,
        rationale: String,
    ...
    /// 2026-08-14): ответ модели был «почти» в протоколе и достроен
    /// детерминированно ДО медиации (плоская форма, `action` строкой,
    /// отсутствующий thought, оборванный JSON). Смысл по-прежнему
    /// решают валидация и гейт — событие фиксирует факт ремонта формы.
    AgentTurnNormalized {
        step_id: String,
        detail: String,
    },
