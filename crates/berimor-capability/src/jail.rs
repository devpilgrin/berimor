...
//!
//! Известное ограничение (честно, для ревью): между проверкой и реальным
//! обращением к ФС существует окно TOCTOU — злоумышленник с параллельным
//! доступом к ФС может подменить симлинк после проверки. Для модели угроз
//! системы (один писатель на инстанс, агент — единственный автор действий
//! в области) это окно не входит в перечень угроз §1; закрытие через
//! `openat2(RESOLVE_BENEATH)` — платформо-специфичная задача за пределами
//! этого milestone.
