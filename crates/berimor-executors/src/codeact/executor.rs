...
fn build_prompt(
    ...
    let mut prompt = format!(
        "Шаг процесса: {step_id}. Напиши ПРОГРАММУ на JavaScript (executors.md §4.1) — не JSON-ответ.\n\n\
         ...
         хоста (гейт, лимиты) НЕ бросает исключение — проверяй `.ok` сам. Исключение бросается \
         только если `args` несериализуемы (функции, циклические структуры) — передавай простые \
         JSON-значения (3.18 аудита: прогон C6 показал расхождение документации с фактом). \
         Доступные для этой программы имена инструментов: {tools_list}.\n\
