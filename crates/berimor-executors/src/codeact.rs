...
//! - **E7** — статический анализ сгенерированной программы (белый список
//!   идентификаторов) и сам JS-движок (`stack.md` §4: встроенный QuickJS,
//!   компилируемый в WASM). Этот модуль исполняет ЛЮБОЙ корректный
//!   WASM-модуль, поданный в [`WasmHost::run`] — не только «программу на
//!   ограниченном подмножестве JS»; статический разбор исходника — забота
//!   вызывающего кода задачи E7, не этого хоста.
...
//!   нечего целиком (нет статического анализа, нет лимитов, нет проводки
//!   результата через Mediation) — это задача E8.
//!
//! ## ABI хост↔гость
//!
