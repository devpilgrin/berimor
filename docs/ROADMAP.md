...
Авторазрешение конфликтов — v3, осознанно.
...
- **LOW-волна**: аудит `docs/audit-2026-07-31.md` размечен ПОЛНОСТЬЮ —
  ...
  *1.12 (паника при ручной сборке ProcessInstance минуя compile).*
  Точки: engine.rs:368/373 (expect «граф гарантирует»), :407, :501.
  Два шага, не один: (а) НЕЛОМАЮЩИЙ — expect → EngineError (вариант
  `InvalidInstance { detail }`): ручная сборка перестаёт паниковать
  даже сейчас; 4 expect + тест «ручной инстанс → ошибка, не паника»;
  (б) ломающий (приватизация полей + конструктор) — отдельно, при
  мажорной версии API. Шаг (а) дёшев и закрывает живой ущерб.

  *1.15 (human_gate журналирует CLI, не движок → resume пишет второй
  HumanGateOpened, пары в online_metrics искажены).* Конечное состояние:
  события гейта пишет ДВИЖОК в обеих точках (остановка и возобновление
  с входом) — единственный писатель, resume и первый прогон сходятся.
  Дешёвая промежуточная мера: при resume не писать Opened, если последнее
  гейт-событие инстанса — незакрытый Opened (дедуп по журналу); метрики
  при этом считать только по сомкнутым парам. Рекомендация: сначала
  дедуп (полдня), перенос в движок — при следующем касании engine API.
