...
- **Волна L (глубокое исследование)** — переоценка ранее отклонённого
  ...
  похожести эмбеддингов. Формат: по каждому пункту формальное
  доказательство отказа уже записано (ADR-0028); исследование — это
  попытка это доказательство ОПРОВЕРГНУТЬ (формализация, прототип,
  измерение), а не подтвердить. Отказ пересматривается только при
  выполнении условий пересмотра из ADR-0028.
- **Волна M (фронтенд)** — визуальный DAG-builder процессов.

Доказательства отказов и условия пересмотра: docs/ADR/0028-wave-l-rejections.md.
