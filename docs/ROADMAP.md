...
Tool Runtime). O1 — чтение журнала для человека/отладки, не
переиспользование `engine::recover` (тот восстанавливает исполняемый
`ProcessInstance` под граф процесса и проверяет версию — другая цель,
«продолжить выполнение», не «посмотреть, что произошло»).
