...
**v2 (следующая волна)**: реестр акторов в демоне (актор на расписание,
конверты между ними), уведомления о конфликтах файлов между сессиями
(заимствование swarm-модели jcode на нашем mailbox — общий журнал
SQLite хоста как транспорт).
...
### 20.23. Провайдер эмбеддингов: fastembed + multilingual-e5-small

**Статус: 🚧 в работе** (2026-08-06, директива «оптимальная
мультиязычная модель даже на CPU»). Задокументированный пробел:
VectorSimilarity принимает `&dyn Fn(&str) -> Vec<f32>`, но эмбеддер
отсутствует — семантическая память работает по точному хэшу.

**Решение** (исследование 2026-08-06): крейт `fastembed` (Qdrant, ONNX
Runtime, CPU-first, без Python) + `intfloat/multilingual-e5-small`
(118M параметров, 384 dim, 100 языков, ONNX-экспорт официальный,
int8 — сотни коротких документов/с на современном CPU). Альтернатива по
качеству — BAAI/bge-m3 (тяжелее, ~2 ГБ) — опциональный профиль.
Feature `embeddings` (выкл. по умолчанию — бинарник не тяжелеет),
модель скачивается при первом включении в data-dir, конфиг
`[memory] embeddings = true, model = "e5-small"`.
...
**Статус: 📋 запланировано (последняя очередь).** Сайд-панель для
файлов/diff'ов, mermaid инлайн. **Тесты**: рендер в pty-smoke с
фиксированным размером (существующая дисциплина script+stty).
