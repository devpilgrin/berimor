...
`crates/berimor-actors`, 380 тестов в воркспейсе на момент последнего
обновления этой секции. A2 и A4 разблокированы (S6 и P7 закрыты), но
ещё не реализованы — следующие по зависимостям, когда до них дойдёт очередь.

| ID | Задача | Сложность | Класс | Зависит от | Источник |
|---|---|---|---|---|---|
| A1 | ✅ Модель актора: задача tokio + почтовый ящик + привязка профиля памяти | L | Сильный | P3, MEM8 | ADR-0009 |
| A2 | Подпись конвертов + проверка ACL топика на шине событий — разблокирована (S6 ✅) | L | Сильный | A1, S6 | `arch/security-model.md` §4 |
