...
хуже существующих агентных клиентов»..
...
- **Волна F (0.43.0)** — berimor как GitHub App: вебхуки в serve,
  HMAC-SHA256, JWT → installation token, ответ комментарием.

### 21.1. Очередь следующих волн (2026-08-24, принято к исполнению)

- **Волна G (0.44.0)** — ACP (Agent Client Protocol): berimor как
  исполнительный контур для редакторов (Zed и др.).
- **Волна H (0.45.0)** — сетевые правила Landlock (ABI v4+): песочница
  контролирует не только файловую систему, но и сеть.
- **Волна I (0.46.0)** — федеративный export/import журнала: переносимая
  история прогонов между машинами.
- **Волна J (0.47.0)** — пентест-скилл в каталог berimor-skills.
- **Волна K (0.48.0)** — TUI: drag за ползунок прокрутки.
- **Волна L (исследовательская)** — переоценка ранее отклонённого
  (возвращено в план решением владельца 2026-08-24): распределённый
  рой/A2A, WASM-снапшоты QuickJS, спекулятивные ветки CodeAct, кэш по
  похожести эмбеддингов. Каждый пункт — сначала spike с измеримым
  выводом, затем решение о реализации; причины первоначального отказа
  сохранены ниже как отправная точка разбора.
- **Волна M (фронтенд)** — визуальный DAG-builder процессов.

Первоначальные причины отказа (для разбора в волне L): рой/A2A ломает
single-node детерминизм; куча JS не снапшотится и программы
короткоживущие; ветки CodeAct выбираются кодом, ценность маргинальна;
кэш по похожести эмбеддингов — недетерминизм ответа.
