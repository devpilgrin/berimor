...
## 11. Фаза 7 — Actors & Scheduler — A1/A3/A5/A6 ВЫПОЛНЕНЫ

`crates/berimor-actors`, 355 тестов в воркспейсе на момент закрытия
этих четырёх задач. A2 остаётся заблокирована S6 (вне этой фазы). A4
разблокирована после P7 (`docs/ROADMAP.md` §9), но ещё не реализована —
следующая по зависимостям, когда до неё дойдёт очередь.

| ID | Задача | Сложность | Класс | Зависит от | Источник |
|---|---|---|---|---|---|
| A1 | ✅ Модель актора: задача tokio + почтовый ящик + привязка профиля памяти | L | Сильный | P3, MEM8 | ADR-0009 |
| A2 | ⛔ Подпись конвертов + проверка ACL топика на шине событий — заблокирована S6 | L | Сильный | A1, S6 | `arch/security-model.md` §4 |
| A3 | ✅ Диспетчер: доска задач, правила назначения, эскалация после N неудач | M | Средний | A1 | `ideal-agent-architecture.md` §3.8 |
| A4 | Лимит очереди `human_gate` + статус `throttled` для расписаний — разблокирована (P7 ✅) | M | Средний | A3, P7 | `arch/process-engine.md` §5, ADR-0015 |
...
## 12. Фаза 8 — Tool Runtime & Protocol (Трек C)

| ID | Задача | Сложность | Класс | Зависит от | Источник |
|---|---|---|---|---|---|
| T1 | MCP-клиент (потребление внешних серверов инструментов) | M | Средний | E1 | `arch/stack.md` §6, ADR-0023 |
| T2 | MCP-сервер (отдача собственных инструментов/ресурсов/шаблонов) | M | Средний | E1 | `arch/stack.md` §6, ADR-0023 |
| T3 | Изоляция плагина как процесса + IPC-транспорт | L | Сильный | T1, S6 | `ideal-agent-architecture.md` §3.9 |

## 13. Фаза 9 — Eval & Observability (Трек D, параллельно с Фазой 2+)
