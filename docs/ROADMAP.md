...
**Дополнение 2026-08-02 (красный CI-windows после пуша)**: первый фикс
C1 опирался на POSIX-семантику `exists()` — на Windows `link\..`
схлопывается лексически, и три теста упали. Алгоритм заменён на
платформенно-независимый покомпонентный обход: каждый встреченный
...
это разрешено принципиально, фикстура обновлена.
