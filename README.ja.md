...
[![Tests](https://img.shields.io/badge/tests-1002%20green-brightgreen)](#プロジェクトのインフラ)
...
**ウェーブE：メモリ** (0.42.0)：Facts 層の Qdrant アダプタ — `[memory] qdrant_url = "http://127.0.0.1:6333"`（+ `qdrant_collection`、`qdrant_api_key_env`）：SQLite 全スキャンに代わる HNSW インデックスのセマンティック検索（純粋 HTTP/JSON、gRPC クライアントなし）。upsert/scroll/cosine/hybrid/delete を実際の Qdrant で実測。完全一致ハッシュのモデル応答キャッシュ — `[agent] response_cache = true`：同一入力の再呼び出しはプロバイダに届かない（ヒットは usage を書かない——呼び出し自体がない）。保存は独立した `<storage>.cache.db`（削除=無効化）。埋め込み類似度キャッシュは意図的に作らない（回答の非決定性）。
...
**ウェーブI: フェデレーテッドジャーナル**（0.46.0）：`berimor journal export <instance> --out <file>` / `import <file>`——sha256 フィンガープリント付きの可搬 JSON で実行をマシン間転送。インポートはハッシュを検証（破損ファイルは拒否）、id 衝突時はリネーム（`-imported-N`）——既存インスタンスへの追記は不可。タイムスタンプは元のまま保持。

**ウェーブK: TUIスクロール**（0.48.0）：ジャーナルのスクロールバーつまみのドラッグが、ボタンを離すまでポインタに追従——狭いトラック列を外れても（以前はドラッグが失われた）。トラッククリックは比例ジャンプ、▲/▼はステップ；ドラッグ座標はトラックにクランプ。（ウェーブJ——0.47.0——はカタログのみ：berimor-skills の `security-pentest-ru` スキル、コアのリリースなし。）

**ウェーブM: ビジュアル DAG ビルダー**（フロントエンド、コアリリースなし）：`tools/dag-builder.html`——単一ファイルのプロセスエディタ（全タイプのステップノード、seq/branch/parallel エッジ、StepKind 準拠のフィールドフォーム、グラフ検証、berimor YAML エクスポート、JSON プロジェクト）。フォーマットサンプル `fixtures/golden/processes/dag-builder/` は製品バイナリで解析可能。
...
**生成途切れへの耐性**（0.35.2）：トークン上限に達したローカルモデルは JSON を途中で切り捨てます（「EOF while parsing」）— 以前は3回のリトライを消費し、エスカレーションでプロセス全体が停止していました。今ではメディエーションの parse 段階が途切れを構造的に補完します（閉じ引用符・括弧のみ。内容は不変、ゴミは拒否のまま）。修復はジャーナルに記録されます（`mediation_parse_repaired`）— リトライとエスカレーションは本当のエラー用に残ります。ローカルプロバイダのコンテキストは 8192 に引き上げられ、設定可能です（`local_ctx_tokens`）。
...
**チェックの規律。** すべてのリリースで：`cargo fmt` + `clippy -D warnings` + `cargo test --workspace`（1002 テスト：ユニット、統合、実バイナリ経由の e2e、プロセスと悪意ある入力のゴールデンフィクスチャ）。重要コンポーネントは必須の独立レビューを通ります。完全な独立監査（`docs/audit-2026-07-31.md`）——**すべての指摘は解決済みか、意図的に文書化済み**。
